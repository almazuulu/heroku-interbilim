...
commands = {
    ...
    'ayilokmot': 'Айыл өкмөтүнө арыз жазуу',
    'rowd': 'Райондук ички иштер бөлүмүнө арыз о решение по его делу',
...
df = pd.read_excel('dbtelegram_bot.xlsx', sheet_name='Сгоревшие дома Лейлек')
...
def markup_inline():
    ...
    markup.add(InlineKeyboardButton(text='✍️ Арыз жазуу', callback_data='aryz_jazuu')) \
        ...
        .add(InlineKeyboardButton('💴 Лейлек - Үй тууралуу Компенсация маалыматы', callback_data='crushact')) \
...
#        bot.send_message(cid, 'Привет!', reply_markup=keyboard)
#
...
#        # aryz_ayil_okmot = types.KeyboardButton('✍️ Айыл өкмөтүнө арыз жазуу')
#        # aryz_rayon = types.KeyboardButton('✍️ Райондук ички иштер бөлүмүнө арыз о решение по его делу')
#        # aryz_prezident = types.KeyboardButton('✍️ Президенттин ыйгарым укуктуу өкүлүнүн атына жазылган арыз')
#        # exit_menu = types.KeyboardButton('Менюдан чыгуу')
#        # markup_reply.add(aryz_ayil_okmot,aryz_rayon, aryz_prezident, exit_menu)
#        # bot.send_message(cid, 'Томонку кнопкаларды басыныз', reply_markup=markup_reply)
#     elif call.data =='rovd':
#         bot.send_message(cid, "Лейлек районунун  РИИБдо № 03-038-2021-000219 сотко чейин иликтөө иштери жүргүзүү боюнча кылмыш иши козголгон"
...
#         message_comp_insrtuction = 'Окмөттөн компенсация берилген сумманы билиш үчүн' \
...
@bot.callback_query_handler(func=lambda call:True)
def call_back_menus_sub_menu(call):
    cid = call.message.chat.id

    bot.answer_callback_query(callback_query_id=call.id, text='Спасибо за честный ответ!')
    answer = ''
    if call.data == 'crushactleilekhouse':
        answer = 'Лейлек рулит!'
    elif call.data == '4':
        answer = 'Вы хорошист!'
    elif call.data == '5':
...
def compensassion_count(message):
    ...
    if not compessasion_list:
        ...
        bot.send_message(cid, f'ЛЕЙЛЕК Сиз жазган ФИО боюнча {len(compessasion_list)} уйго компенсация бар')
